...
function ajaxHeart(e) {
  ...
  e.preventDefault();
  console.log("hearted");
  axios__WEBPACK_IMPORTED_MODULE_0___default().post(this.action) //action is the url we wanna hit
  .then(function (res) {
    //make heart 实时
    var isHearted = _this.heart.classList.toggle("heart__button--hearted"); //in button heart'name in storeCard


    (0,_bling__WEBPACK_IMPORTED_MODULE_1__.$)(".heart-count").textContent = res.data.hearts.length;

    if (isHearted) {
      _this.heart.classList.add("heart__button--float"); //need to remove it so we wont click random wrong el
...
function loadPlaces(map) {
  ...
  axios__WEBPACK_IMPORTED_MODULE_0___default().get("/api/stores/near?lat=".concat(lat, "&lng=").concat(lng)).then(function (res) {
    var places = res.data; // console.log(places);
    ...
    var bounds = new google.maps.LatLngBounds(); //info window
    ...
    var markers = places.map(function (place) {
      ...
      marker.place = place;
      return marker;
    }); //when someone clicks on a marker, show the details of place
    ...
    //bounds is like a circle around the pins
...
function makeMap(mapDiv) {
  ...
  loadPlaces(map); //hook up geo location

  var input = (0,_bling__WEBPACK_IMPORTED_MODULE_1__.$)('[name="geolocate"]');
  var autocomplete = new google.maps.places.Autocomplete(input); //can search places and load the lat and lng of which place in that city
...
function typeAhead(search) {
  ...
  var searchResults = search.querySelector(".search__results"); //listen for an input event on the search
  //on = addEventListener
  ...
  searchInput.on("input", function () {
    ...
    searchResults.style.display = "block"; //to chnage searching content when we backspace(deleting)
    ...
    axios__WEBPACK_IMPORTED_MODULE_0___default().get("/api/search?q=".concat(this.value)).then(function (res) {
      if (res.data.length) {
        ...
        var html = searchResultsHTML(res.data); //santize the innerHTML so it wont get hacked by the search result
  ...
  searchInput.on("keyup", function (e) {
    ...
    var activeClass = "search__result--active"; //eveytime we press down will find the current one (which one has curr class)
...
(0,_modules_map__WEBPACK_IMPORTED_MODULE_4__["default"])((0,_modules_bling__WEBPACK_IMPORTED_MODULE_1__.$)("#map")); //use it and give it an id of map
